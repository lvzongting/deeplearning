...
w_conv1 = tf.Variable(tf.truncated_normal([11,11,3,48],stddev=0.01))
b_conv1 = tf.Variable(tf.constant(0.01,shape=[48]))
...
#biLSTM[0][-1].shape = [?,56]
w_fc2    = tf.Variable(tf.random_normal([1552,101],stddev=0.01))
...
for i in range(2000000):
    ...
    if i%30 == 0: 
        ...
        s1 = O+str(i)+':loss:  '+str(loss  )+W
        s2 = W+str(i)+':tr_acc:'+str(tr_acc)+W
        s3 = G+str(i)+':ts_acc:'+str(ts_acc)+W
        print s1,s2,s3
        print ts_accs
        #f1.write(s1+s2+s3+'\n')
        #f1.flush()
