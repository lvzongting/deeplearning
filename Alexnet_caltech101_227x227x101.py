...
w_fc9   = tf.Variable(tf.truncated_normal([1000,101],stddev=0.1),name='t_w_fc9')
b_fc9   = tf.Variable(tf.constant(0.1,shape=[101]),name='t_b_fc9')
y       = tf.nn.softmax(tf.matmul(fc8,w_fc9)+b_fc9)

allvars = tf.trainable_variables()
tvars   = [v for v in allvars if v.name.startswith('t_')]
#print tvars
#raw_input('wait')

cross_entropy = tf.reduce_mean(-tf.reduce_sum(y_ * tf.log(y),reduction_indices=[1]))
train_step    = tf.train.AdamOptimizer(0.001).minimize(cross_entropy,var_list=tvars)
