...
lstm1_init_state = lstm1_cell.zero_state(batch_size=1000,dtype=tf.float32)
...
#for all_num in enumerate(tf.python_io.tf_record_iterator('caltech101/all_L.tfrecords'))
#all_num = 

all_img, all_label = load_tfrecords('caltech101/all_L.tfrecords')
img_batch, label_batch = tf.train.shuffle_batch([all_img, all_label], batch_size=1000, capacity=2000, min_after_dequeue=1000)
...
ts_batch = sess.run([img_batch, label_batch])
tr_batch = sess.run([img_batch, label_batch])
print ts_batch[0].shape,ts_batch[1].shape

for i in range(20000):
    sess.run(train_step, feed_dict={x: tr_batch[0],y0:tr_batch[1]}) 
    print(str(i)+':'+str(sess.run(accuracy,feed_dict={x: ts_batch[0], y0: ts_batch[1]})))
